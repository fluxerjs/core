...
import { dispatchForTest, fixtureGuild } from '../TestKit/Fixtures.js';
...
describe('Client gateway helpers and dispatch', () => {
  ...
  it('applies configured cache limits', () => {
    ...
    for (const manager of [client.guilds, client.channels, client.users]) {
      ...
      expect(Array.from(manager.keys())).toEqual(['2']);
    }
  });
  ...
  it('keeps the last known counts when a repeated READY omits them', () => {
    const existing = new Guild(client, fixtureGuild({ member_count: 120, online_count: 17 }));
    ...
    hydrateReadyGuilds(
      client,
      [fixtureGuild({ name: 'Counted guild', member_count: 140, online_count: 22 })],
      false,
    );
    expect(client.guilds.get(existing.id)).toMatchObject({
      memberCount: 140,
  ...
  it('updates cached guild counts from GUILD_COUNTS_UPDATE', async () => {
    ...
    await dispatchForTest(client, 'GUILD_COUNTS_UPDATE', {
      counts: [{ guild_id: guild.id, member_count: 140, online_count: 7 }],
    });

    expect(guild).toMatchObject({ memberCount: 140, onlineCount: 7 });
    expect(emit).toHaveBeenCalledWith(Events.GuildCountsUpdate, {
      counts: [{ guildId: guild.id, memberCount: 140, onlineCount: 7 }],
    });
  });
  ...
  it('handles GUILD_MEMBERS_CHUNK by caching members and emitting GuildMembersChunk', async () => {
    ...
    await dispatchForTest(client, 'GUILD_MEMBERS_CHUNK', payload);
  ...
  it('ignores invalid members in GUILD_MEMBERS_CHUNK and caches valid ones', async () => {
    ...
    await dispatchForTest(client, 'GUILD_MEMBERS_CHUNK', {
      guild_id: 'g2',
      chunk_index: 0,
      chunk_count: 1,
      members: [
  ...
  it('defers MESSAGE_CREATE until Ready when waitForGuilds is true', async () => {
    ...
    await dispatchForTest(client, 'MESSAGE_CREATE', {
      id: 'm1',
      channel_id: 'c1',
      guild_id: 'g1',
      content: 'hi',
