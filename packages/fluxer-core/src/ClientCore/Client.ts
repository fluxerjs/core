...
import { type ClientOptions, resolveCacheLimits } from '../Helpers/Options.js';
...
export class Client extends EventEmitter {
  ...
  /** @internal */
  _messageCacheStats(): { channels: number; messages: number } {
  ...
  requestGuildMembers(options: {
    ...
    if (options.userIds !== undefined) data.user_ids = [...new Set(options.userIds)];
    if (options.presences !== undefined) data.presences = options.presences;
    if (options.nonce !== undefined) data.nonce = options.nonce;
    this._sendToAllShards({ op: GatewayOpcodes.RequestGuildMembers, d: data });
  }
