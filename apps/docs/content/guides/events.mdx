...
`GuildCreate` means the bot joined a guild after Ready (invite / add). Guilds listed as unavailable stubs in READY are inserted into `client.guilds` with `available === false`; when their full `GUILD_CREATE` snapshot arrives, the same instance is refreshed and `GuildAvailable` fires instead. Gateways that still send an empty READY list get the same treatment for a short hydration window after Ready. Use `GuildAvailable` (or Ready + `client.guilds`) for startup hydration, not `GuildCreate`.

To restore the old behavior where startup hydrations also emitted `GuildCreate`, set [`ClientOptions.emitGuildCreateOnStartup`](/guides/upgrading-to-3-1/):
