...
- **Identify READY**: unavailable guild stubs are inserted (or existing guilds marked unavailable); full snapshots arrive as GUILD_CREATE and reuse the same `Guild` instance
- **Unavailable** (`GUILD_DELETE` with `unavailable: true`): guild stays cached with `available === false`; the next available GUILD_CREATE recovers the **same** guild instance and syncs nested caches (`GuildAvailable`)
- Permanent **GUILD_DELETE**: guild removed; channels + message caches cascade away
- Startup / reconnect backfill emits `GuildAvailable`; `GuildCreate` is for joining after Ready (unless `emitGuildCreateOnStartup: true`)
