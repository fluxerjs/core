...
description: Breaking changes for the Fluxer.js 1.x → 2.0 rewrite. For 2.x → 3.0, see Upgrading to 3.0.
category: upgrading
order: 1
---

<Warning>
This is the 1.x to 2.0 rewrite. If you are already on 2.x, use [Upgrading to 3.0](/guides/upgrading-to-3/) instead.
</Warning>

Already on 2.x and moving to 3.0? Use [Upgrading to 3.0](/guides/upgrading-to-3/) instead.

New to Fluxer? Follow [Installation](/guides/installation/) → [Basic bot](/guides/basic-bot/) → [Prefix commands](/guides/prefix-commands/) → [Errors](/guides/errors/) → [Caching](/guides/caching/). Coming from discord.js? See [From discord.js](/guides/from-discordjs/).
...
8. When you are ready for current APIs, continue with [Upgrading to 3.0](/guides/upgrading-to-3/).
...
`APIInvite` is a **discriminated union** (guild / group DM). Do not assume `guild` and `channel` always exist. Pack invite types (`InviteType.EmojiPack` / `StickerPack`) last through 2.2 and are removed in [3.0](/guides/upgrading-to-3/#packs-flags-and-overwrites).
...
Also available: `createEmoji` / `cloneEmoji` / `createSticker` / `cloneSticker`. See [Emojis & Stickers](/guides/emojis/).
...
- Read state / preload / apps: `channel.clearReadState()`, `client.preloadMessages(ids)`, `client.fetchApplication()`, `fetchOAuthApplications()` (bot token returns the bot application object)
...
| `preloadMessages` / search / webhook / discovery options camelCase | `@fluxerjs/core` | Wire at REST edge |
...
| `APIInvite` is guild / group-DM union | `@fluxerjs/types`, `@fluxerjs/core` | `isGuild()` / `isGroupDM()` |
...
| Role create is name, color, permissions | `@fluxerjs/core` | hoist / mentionable on edit; unicodeEmoji read-only |
...
| `GuildRoleUpdate` emits `(oldRole, role)` | `@fluxerjs/core` | Was payload object / bare `Role` |
| `GuildRoleDelete` emits `(role, guildId, roleId)` | `@fluxerjs/core` | `role` is cached `Role` or `null` |
| `GuildMemberUpdate` old is `null` when uncached | `@fluxerjs/core` | Never equals new |
| Audit/vanity/discovery returns camelCase | `@fluxerjs/core` | Not raw `API*` where wrapped |
...
- Moving on from 2.x? [Upgrading to 3.0](/guides/upgrading-to-3/).
